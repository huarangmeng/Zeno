...
编译器从包根目录向上查找最近的 `Zeno.toml`。多包 workspace、依赖解析和 lockfile 规则见 [PACKAGE.md](PACKAGE.md)。
...
- `version` 可选，推荐语义化版本。
- `kind` 可选，取值为 `"application"` 或 `"library"`；stage0 可以按 `src/main.zn` 推断，但发布和 workspace 构建建议显式写出。
...
第一版实现固定以下默认值，除非 manifest 显式覆盖且 stage0 已实现对应能力：
...
stage0 可以解析 `panic.strategy = "unwind"`，但在 unwind cleanup、personality、目标 ABI 和 MIR cleanup verifier 完成前，应给 staged diagnostic，而不是生成半成品 unwind。`panic.stack = "symbols"` 只能在 hosted debug 或目标显式支持符号化时生效；否则降级为 `"addresses"` 或按严格模式报错。
...
- dependency key 不能和当前包 `src/` 顶层目录、内建包根或其他依赖 key 冲突。
- 包依赖图不能有环。
- `package.kind` 若出现，必须是 `"application"` 或 `"library"`；`kind = "application"` 必须能解析入口函数，`kind = "library"` 不能要求入口函数。
