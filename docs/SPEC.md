...
资源拥有值不会隐式复制。非 `Copy` 值进入拥有位置时会转移所有权，移动资源会让原绑定不可再用。普通函数参数 `x: T` 不触发所有权转移；它是只读访问。接收所有权的参数通过声明处的 `move x: T` 表达，消费方法通过 `move self` 表达。

Zeno 采用混合显式移动规则：

- 赋值、`let` 绑定、`return`、结构体字段初始化、enum payload 构造等拥有位置可以自动移动非 `Copy` 值，因为语法本身已经在构造新的拥有者。
...
`move` 不是普通表达式前缀。它只出现在参数声明、`move self`、调用实参、消费接收者、闭包捕获、`match move`、`if let move`、`while let move` 和 `for move` 这类所有权模式位置。`return move value` 和普通 `let owner = move value` 是无效语法；这些拥有位置直接写 `return value`、`let owner = value`。
...
- 非 `Copy` 拥有者进入拥有位置时自动移动，例如 `let b = packet;`。之后继续使用 `packet` 是错误。
...
```zn
let old = items.replaceAt(i, move user);
...
let counts = Map<U64, USize>.withCapacity(128); // 哈希映射，默认 allocator
let seen = Set<U64>.withCapacity(128);          // 哈希集合，默认 allocator
...
- 普通函数的所有权转移由被调用函数签名中的 `move` 参数和调用点 `move` 共同表达：`consume(move bytes)` 会把 `bytes` 移给 `consume(move data: Vector<U8>)`。消费方法通过方法签名中的 `move self` 和调用点 `move receiver.method()` 共同表达。
...
- `Map.remove(key) -> Option<V>` 移除 key 对应的 value，并销毁表内 key。
- `Map.removeEntry(key) -> Option<(K, V)>` 移除并返回表内 key 与 value。
- `Set.insert(move value) -> Bool` 调用签名接收 value 所有权，返回是否真正插入；已有等价值时销毁传入 value。
- `Set.remove(value) -> Bool` 移除并销毁表内值。
- `Set.take(value) -> Option<T>` 移除并返回表内值。

`Map.get(key) -> Option<V>` 只为 `V: Copy` 提供，因为它返回 value 的复制。非 `Copy` value 的短期访问使用 checked index 语义：

```zn
if counts.containsKey(id) {
    let count = counts[id];        // USize 是 Copy，复制值
}
...
`Map.entry(mut self, move key)` 的签名接收 key 所有权，返回 view-like 的 `MapEntry<K, V>`，用于单次探测后插入、更新或移除。`MapEntry<K, V>` 不能保存到结构体、集合、`static`、`Box`、`Shared`、逃逸闭包、线程、任务或 async future 中。

```zn
let entry = counts.entry(id);
if entry.exists() {
...
```zn
let byte = bytes[i];          // T: Copy 时复制元素值
let id = users[i].id;         // 非 Copy 元素上读取 Copy 字段，不移动元素
...
`if let` 和 `while let` 使用第 15 节的 pattern 系统：
...
while let move Some(item) = iterator.next() {
    consume(move item);
}
```

- `if let pattern = expr` 在 pattern 匹配时执行 then 分支，不匹配时跳过或进入 `else`。
- `while let pattern = expr` 每次循环先求值并匹配，匹配时进入循环体，不匹配时结束循环。
- `if let move pattern = expr` 和 `while let move pattern = expr` 消耗匹配值。
- `if let mut pattern = place` 和 `while let mut pattern = place` 要求右侧是可写位置，并给 payload 提供唯一可写访问。
- `let pattern = expr` 只允许不可失败 pattern；可能失败的 pattern 必须写 `match`、`if let` 或 `while let`。
...
- `let pattern = expr`，仅限不可失败 pattern

`match` 必须穷尽，除非存在通配分支。`match`、`if let` 和 `while let` 的接收模式决定被匹配值内部绑定的所有权：
...
- `if let` 和 `while let` 支持相同模式：`if let move Some(x) = value`、`if let mut Ready(job) = state`。
- `let pattern = expr` 没有 `move` / `mut` 前缀；它根据右侧表达式的普通所有权规则初始化绑定。右侧是命名非 `Copy` 值并需要所有权时会自动移动。
...
- 带自定义 `destroy` 块的 struct 或 enum 在安全代码中不能通过 `match move` 或 `let` 解构移动出非 `Copy` 字段 / payload；只能读取、可写访问，或通过受审计的 `trust` / 核心库 primitive 拆解。
...
`let` 解构只允许不可失败 pattern：

```zn
let Point { x, y } = point;       // ok
let (left, right) = pair;         // ok
let Some(value) = maybe;          // error: refutable pattern in let
```

不可失败 pattern 包括单个绑定、通配、tuple 的不可失败子 pattern、struct 的字段 pattern，以及对当前类型唯一可能 variant 的 pattern。普通多 variant enum 的 variant pattern 是可失败 pattern，必须写 `match`、`if let` 或 `while let`。
...
let bytes = move packet.intoBytes(); // 消耗；packet 不可再用
```
...
fn tee<W: Writer>(mut left: W, mut right: W, bytes: ArraySlice<U8>) -> Result<Unit, IoError> {
    ...
    try left.write(bytes);
    try right.write(bytes);
    return Ok(());
}
```
...
fn duplicateBox(value: Box<Duplicable>) {
    let copy = value.duplicate(); // error: 返回 Self 的方法不能动态派发
}
```
...
- 跨 `await` 存活的非 `Copy` 值必须由 future 拥有；短期只读访问、`mut` 访问和视图值不能跨 `await`。
- 在 async 函数中注册且跨过 `await` 的 `defer` 会进入 future 状态；正常返回、`try` 提前返回、panic-unwind 和 future drop 都必须按作用域规则执行它。
- future drop 不能 `await`，因此跨 `await` 的 `defer` 不能包含 `await`，也不能依赖异步清理完成。
- `ArraySlice<T>`、`StringSlice` 和其他非拥有访问值不能跨 `await` 进入 future 状态；如果只在 await 前使用并结束访问，则允许。接口约束按具体类型处理，跨 `await` 时必须由 future 拥有该具体值。
- v1 不需要用户可见 `Pin`。禁止访问值跨 `await` 后，future 在没有被 runtime 拥有时可以按普通 move 规则移动。
