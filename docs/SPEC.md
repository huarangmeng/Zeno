...
`move` 不是普通表达式前缀。它只出现在参数声明、`move self`、调用实参、消费接收者、闭包捕获、`match move`、`if (expr is move pattern)`、`while (expr is move pattern)` 和 `for move` 这类所有权模式位置。`return move value` 和普通 `val owner = move value` 是无效语法；这些拥有位置直接写 `return value`、`val owner = value`。
...
`if (expr is pattern)` 和 `while (expr is pattern)` 使用第 15 节的 pattern 系统：
...
while (iterator.next() is move Some(item)) {
    consume(move item);
}
```

- `if (expr is pattern)` 在 pattern 匹配时执行 then 分支，不匹配时跳过或进入 `else`。
- `while (expr is pattern)` 每次循环先求值并匹配，匹配时进入循环体，不匹配时结束循环。
- `if (expr is move pattern)` 和 `while (expr is move pattern)` 消耗匹配值。
- `if (place is mut pattern)` 和 `while (place is mut pattern)` 要求被匹配值是可写位置，并给 payload 提供唯一可写访问。
- `val pattern = expr` 只允许不可失败 pattern；可能失败的 pattern 必须写 `match`、`if (expr is pattern)` 或 `while (expr is pattern)`。
...
`match` 必须穷尽，除非存在通配分支。`match`、`if (expr is pattern)` 和 `while (expr is pattern)` 的接收模式决定被匹配值内部绑定的所有权：
...
- `if` pattern 和 `while` pattern 支持相同模式：`if (value is move Some(x))`、`if (state is mut Ready(job))`。
...
不可失败 pattern 包括单个绑定、通配、tuple 的不可失败子 pattern、struct 的字段 pattern，以及对当前类型唯一可能 variant 的 pattern。普通多 variant enum 的 variant pattern 是可失败 pattern，必须写 `match`、`if (expr is pattern)` 或 `while (expr is pattern)`。
