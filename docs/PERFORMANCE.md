...
默认零基线运行时是这些原则在启动、链接和静态存储上的具体化。Zeno 程序只为实际可达的能力付费：

- 语言本身不要求全局运行时、GC、executor、线程池、反射表、异常运行时、全局注册表或模块初始化函数。
- profile 只表示能力可用，不表示能力会自动链接、自动初始化或自动占用内存。
- `core` 必须能单独支撑 freestanding 程序；`alloc`、hosted `std`、线程、任务运行时、格式化、反射和符号化都按可达性进入产物。
- `import` 只影响名字解析和依赖可达性，不执行代码。
- `static` 必须由 CTFE 物化为静态数据；禁止隐式动态全局初始化、全局构造函数、全局析构函数和隐藏 `atexit` 链。
- 运行期初始化必须出现在源码可执行路径中，例如 `main`、显式 `init` / `open` / `load` / `new` API、`Runtime.withWorkerCount` 或 `Once<T>`。
- 标准库声明必须记录运行时需求，例如默认 allocator、OS 线程、任务运行时、调用栈符号化、反射元数据和高级格式化器。
- driver / linker 必须基于全程序可达性只链接可达需求；库不能因为被 import 就启动后台线程、注册全局对象或拉入大型模块。
- 构建工具应提供成本解释输出，说明默认 allocator、线程、任务运行时、unwind、调用栈符号化、反射元数据或高级格式化器为什么进入产物。
...
- 把 `import` 降低成模块初始化函数或全局注册代码。
- 生成隐式动态 `static` 初始化函数、全局构造函数、全局析构函数或隐藏 `atexit` 注册。
- 因为使用 hosted profile 就自动链接线程池、任务运行时、反射元数据、符号化器或大型格式化模块。
- 为普通整数 `+`、`-`、`*` 生成隐藏溢出检查。
...
- 默认错误流不生成异常机制。
- 默认 panic/OOM 只污染冷路径。
- 不使用某个运行时需求时，release 产物中不出现对应初始化路径、全局状态或后台线程。
- `static` 只产生静态数据或目标支持的静态初始化记录，不产生启动前执行的用户代码。
