...
- SourceManager 统一管理源码文本、文件身份、字节 offset、行列映射和 diagnostic span。
- Lexer 只输出 token、trivia 边界和 lexical diagnostic。
- Parser 只输出 AST 和语法恢复诊断；AST 只表达语法结构、token/span 和源码顺序。
- Declaration Collection 收集顶层声明签名、模块路径、可见性、重载集合入口和 stable node id。
- Name / Module Resolution 完成名字、模块、可见性、import、同包直接可见和重载候选解析。
- HIR 表达解析后的源码语义：类型骨架、接口、泛型、属性、`val` / `var`、`mut` / `move`、`trust` 和 profile 影响。
- Type / Interface / Ownership Sema 完成类型、接口约束、方法接收者、访问模式、初始化、move、逃逸、`Send` / `Sync`、trust 能力、layout 和 CTFE 检查。
- MIR 表达显式控制流和低层语义，是主要优化 IR。`try`、RAII、panic/OOM 终点、drop flags、cleanup edges、bounds checks、alias facts 和 ownership facts 都必须在 MIR verifier 中可检查。
- Monomorphization 把泛型函数、泛型类型和静态接口约束实例化为具体类型；mono key 必须包含约束证明、layout fingerprint、target 和 profile。
- LLVM IR 只接收已经证明安全和优化合法的低层事实，不能承担 Zeno 语言安全检查。
...
- const context 标记、CTFE 依赖、常量参数值和常量物化策略。
...
- 使用点请求具体 mono item。
- 常量泛型实参必须先经 CTFE 得到稳定 value fingerprint。
- HIR/Sema 阶段完成泛型约束检查、静态接口候选收集和 mono item planning。
- MIR 阶段必须先通过 verifier，再进行具体 mono item emission；LLVM lowering 不接收未经过 MIR verifier 的泛型实例。
- 静态接口约束调用在 mono emission 中解析为具体函数或可内联调用。
...
- 只有源码类型显式使用接口拥有者 `Box<Interface>` 或 `Shared<Interface>` 时，mono emission 才保留接口表派发。
...
CTFE 是编译期执行普通 Zeno 代码的解释器。它运行在 typed HIR 上，必要时可以复用 MIR 解释器实现，但不能依赖 LLVM 优化结果。

CTFE 必须支持：

- 普通函数、方法、泛型函数和静态接口派发。
- `if`、`while`、`for`、`match`、`break`、`continue`、`return`。
- 结构体、枚举、元组、闭包、集合临时值和 pattern。
- move、初始化、drop flag、RAII 和 `destroy`。
- 编译期 arena 分配，用于构造编译期临时 `String`、`Array`、`Vector`、`Map` 和 `Set`。

CTFE 必须拒绝：

- `trust`、extern 调用、裸内存、硬件访问、inline asm 和中断入口。
- 线程、任务、`await`、阻塞等待和运行期同步等待。
- 当前时间、随机数、环境变量、进程状态、网络和未声明文件输入。
- 修改运行期 `static` 存储，或依赖运行期地址身份。

CTFE 产出的值分为：

- 纯编译期值：只用于常量折叠、属性、泛型实参和编译期判断。
- 可内联运行期值：`Copy` 标量和纯 `Copy` 聚合。
- 只读静态数据：可作为 `StringSlice` 或 `ArraySlice<T>` 的底层存储。
- 需要显式物化的拥有者：`String`、`Array<T>`、`Vector<T>`、`Map<K, V>`、`Set<T>`、`Box<T>` 和 `Shared<T>` 不能隐式变成运行期堆对象。

CTFE key 必须包含被执行函数 fingerprint、常量实参 fingerprint、target、profile、layout、panic/OOM、allocator、trust 配置、compiler identity 和声明的 build input hash。CTFE 诊断必须保存编译期调用栈并可从缓存重放。
...
- `if (expr is pattern)` 降低为 refutable pattern test 加 then / else 分支；`while (expr is pattern)` 降低为循环头 pattern test 加 body / exit 分支。
...
`async fn`、`async { ... }` / `async move { ... }` block，以及 `Future<T>` 参数位置的 Future block 实参降低为拥有状态机。MIR 必须显式表示：
...
- `Runtime.spawn(move future)` 和 `Runtime.spawn({ ... })` 应 lowered 为显式 task control block enqueue；用户代码移动的是 `Task<T>` 句柄，不是已经交给 runtime poll 的内部 future 状态。Future block 实参不允许额外生成普通 closure heap allocation。
- `Runtime.spawnWithContext(makeFuture)` 应 lowered 为“创建任务上下文，调用一次 `makeFuture(ctx)`，把返回的 future 入队”；上下文指向 runtime 任务控制块中的取消状态。runtime 不保存 `makeFuture` 闭包。`makeFuture` 的 block body 若使用 Future block 简写，不生成额外普通 closure allocation。
- `Runtime.spawnBlockingWithContext(job)` 应 lowered 为显式同步任务上下文参数；`spawnBlocking(job)` 不创建用户可见 `TaskContext`。
- `TaskGroup.joinAll` / `tryJoinAll` 应 lowered 为 group 拥有的完成队列 drain。默认完成顺序收集不能为每个结果引入隐藏堆分配；有序收集的索引和结果槽位来自 `joinAllOrdered` / `tryJoinAllOrdered` 的显式语义。
- `TaskGroup.next` / `tryNext` 的 `None` 路径和 `tryNext` / `tryJoinAll` 的 `Err` 路径必须在 MIR resource state 中把 group 标记为 settled，供 definite cleanup 检查使用。
...
- `@export("symbol", bridge: C)` 生成一个外部 C ABI thunk；原 Zeno 函数仍按普通 Zeno ABI 编译。
- `pub` 不等于导出符号。
- `trust extern "C"` 导入必须经过 C-compatible 签名检查。
- C ABI 边界不能让 panic unwind 穿过。
- `@layout(C)` 和 C-compatible 类型检查在 HIR 完成，MIR 不再接受不合法 C ABI。
- 导出函数不能是泛型函数，不能是方法，不能是闭包，不能暴露非 `pub` 命名结构体类型。
- `bridge: C` 导出函数不能是泛型函数，不能是方法，不能是闭包；其源码签名必须是 bridge-compatible，并且生成 thunk 的低层签名必须是 C-compatible。
- `bridge: C` thunk 只允许零分配、零复制、零动态派发转换：C-compatible 类型直传，`ArraySlice<T>` / `mut ArraySlice<T>` 降为 pointer + length，`StringSlice` 降为 bytes + length，`Result<T, E>` 返回降为 status + out 参数，`Option<T>` 返回降为 found + out 参数。
- `Result<T, E>` bridge lowering 必须调用 `E.toCCode()` 或等价静态 lowering，把错误映射到 `I32` status；缺失 `core.ffi.CErrorCode` 实现时在 HIR 阶段拒绝。
- `bridge: C` 不得为 `String`、`Array<T>`、`Vector<T>`、`Box<T>`、`Shared<T>`、接口拥有者、闭包或带 `destroy` 的资源类型生成隐式分配、复制、释放或布局暴露。
- `@layout(C)` 的 size、align、字段 offset 和 aggregate passing ABI 必须来自 target data layout / C ABI，并进入 mono item key、ABI fingerprint 和增量缓存 key。
- 没有 `@layout(C)` 的普通 struct、普通 enum、`Option`、`Result`、`Char` 和带 `destroy` 的资源类型在 HIR 阶段直接判为非 C-compatible。
- `bridge: C` 生成的 C 头文件片段、thunk 低层签名、错误码映射和目标 C ABI 必须进入 ABI fingerprint 和增量缓存 key。
- `zeno bindgen c` 生成的 raw/safe 源码必须把 header hash、include path、宏定义、target triple、目标 C ABI、Clang 资源目录、生成器版本和 Zeno 版本写入包元数据；这些信息参与增量缓存 key。
- 后续 `zeno bindgen cxx` 必须作为独立 bindgen 前端进入同一缓存框架。C++ 默认通过 C shim、opaque handle、显式模板实例化和异常转换接入，不允许把 C++ mangled name、异常 ABI 或类布局作为未声明的 Zeno ABI 事实。
