...
- const-evaluated HIR 执行 `const`、`static` 初始化、常量泛型实参、属性参数和布局查询所需的 CTFE。
...
- `Runtime.spawn(move future)` 应 lowered 为显式 task control block enqueue；用户代码移动的是 `Task<T>` 句柄，不是已经交给 runtime poll 的内部 future 状态。
- `Runtime.spawnWithContext(makeFuture)` 应 lowered 为“创建任务上下文，调用一次 `makeFuture(ctx)`，把返回的 future 入队”；上下文指向 runtime 任务控制块中的取消状态。runtime 不保存 `makeFuture` 闭包。
