...
- 可达运行时需求摘要，例如默认 allocator、OS 线程、任务运行时、unwind、调用栈符号化、反射元数据和高级格式化器。
...
| 错误码 | 类别 | 含义 |
| --- | --- | --- |
| `E0001` | lexer | 非法字符或非法 token。 |
| `E0002` | lexer | 字符串、字符或注释未闭合。 |
| `E0003` | parser | 缺少必要 token。 |
| `E0004` | parser | 当前位置不允许该语法。 |
| `E0101` | module | `module` 声明与文件路径不匹配。 |
| `E0102` | import | 找不到 import 根、package 或模块。 |
| `E0103` | package | package graph 或 workspace 结构无效。 |
| `E0201` | names | 找不到名字。 |
| `E0202` | visibility | 访问了不可见的 `private` 或非 `pub` 项。 |
| `E0203` | overload | 重复声明同一重载 key。 |
| `E0301` | type | 类型不匹配。 |
| `E0302` | overload | 没有可用重载。 |
| `E0303` | overload | 重载解析不唯一。 |
| `E0304` | conversion | `as` 只能表达无损转换。 |
| `E0305` | effect | `@noAlloc` 中调用了可能分配的 API。 |
| `E0306` | effect | `@noPanic` 中调用了可能 panic 或 OOM-as-panic 的 API。 |
| `E0401` | ownership | 使用了已经 move 的值。 |
| `E0402` | ownership | 命名非 `Copy` 值传给 `move` 参数时缺少调用点 `move`。 |
| `E0403` | initialization | `val` 初始化后被重新赋值。 |
| `E0404` | initialization | 使用了未初始化或可能未初始化的值。 |
| `E0405` | initialization | 聚合值在所有字段初始化前被使用。 |
| `E0406` | destroy | `destroy` 中出现不允许的 `try`、`await`、panic、分配或直接调用。 |
| `E0501` | access | 唯一可写访问与现存访问重叠。 |
| `E0502` | escape | 返回或保存了指向局部存储的访问值。 |
| `E0503` | escape | `ArraySlice`、`StringSlice` 或其他访问值被保存到长期位置。 |
| `E0504` | access | `mut self` 或 `mut` 参数需要可写接收者。 |
| `E0601` | generics | 泛型约束不满足。 |
| `E0602` | interface | 接口对象调用了不能动态派发的方法。 |
| `E0603` | interface | 静态接口返回不能统一到一个隐藏具体类型。 |
| `E0701` | trust | 低层操作、裸 FFI、裸内存或硬件访问缺少 `trust` 边界。 |
| `E0702` | trust | manifest 未授予当前 `trust` 能力。 |
| `E0703` | abi | C ABI / bridge 签名不是 C-compatible。 |
| `E0704` | layout | layout 属性或 C layout 字段非法。 |
| `E0801` | static | `static` 初始化不是可物化 CTFE 数据，或隐藏了运行期初始化 / 全局析构。 |
| `E0802` | ctfe | 编译期求值失败。 |
| `E0803` | ctfe | 编译期常量索引、除法、转换等产生确定非法结果。 |
| `E0901` | mir | MIR verifier 发现 cleanup、drop flag、alias 或 bounds 事实不成立。 |
| `E0902` | codegen | codegen verifier 发现隐藏分配、异常 lowering、隐藏动态派发或错误 LLVM 属性。 |
| `E1001` | manifest | manifest 字段未知或类型错误。 |
| `E1002` | manifest | profile、target、panic/OOM 或 allocator 配置无效。 |
| `E1003` | lockfile | lockfile 与 frozen 构建不一致。 |
| `E9001` | staged | `async` / Future lowering 当前 stage0 未实现。 |
| `E9002` | staged | registry / git dependency 当前 stage0 未实现。 |
| `E9003` | staged | panic unwind 当前 stage0 未实现。 |
| `E9004` | staged | 跨 package opaque interface return metadata 当前 stage0 未实现。 |

后续可以在各分段中追加新错误码，但不能复用或改变已经冻结的错误码含义。错误文案可以优化，测试和工具优先依赖稳定 `code`。
...
- `--explain-cost` 输出可达运行时需求和主要成本来源，例如 allocator、线程、任务运行时、unwind、调用栈符号化、反射元数据或高级格式化器为什么进入产物。
...
- `--milestone` 按测试元数据过滤，主要用于开发中只跑 M0-M9 的局部门禁。
