...
- `stage` 表示测试是否进入当前发布门禁。取值为 `mvp` 或 `full-spec`。
- `milestone` 表示首个应当通过该测试的实现里程碑，例如 `M1`、`M4` 或 `M8`。它只用于开发计划和过滤，不改变语言语义。
- `feature` 表示主要覆盖能力，例如 `ownership`、`parser`、`diagnostic`、`static-init`、`ffi`。
- `profile` 表示测试需要的目标能力，例如 `freestanding`、`hosted`、`kernel`。
- 没有显式 `stage` 的测试默认按 `full-spec` 处理，不进入第一批 MVP 门禁。
...
stage0 开发开始前，仓库必须至少有一批带 `stage: mvp` / `stage = "mvp"` 的核心规格测试。第一批 MVP 标记用于驱动编译器骨架和语义实现；随着 M0-M9 推进，可以把更多完整规格测试提升为 MVP 门禁，但不能把尚未实现的延后能力混入第一批默认门禁。
...
- 新增或被标记为 `stage: mvp` 的失败测试应写稳定错误码，例如 `// expected-error[E0401]: use of moved value`。
...
- `if (expr is pattern)` / `while (expr is pattern)` 支持只读、`move` 和 `mut` pattern 模式。
...
- 隐式动态 `static` 初始化。
- `static` 初始化中执行默认 allocator 分配、运行期 I/O、FFI、线程启动、任务运行时创建，或要求退出时全局析构。
...
- `if (expr is pattern)` / `while (expr is pattern)` 应降低为普通分支 / 循环头，不分配。
